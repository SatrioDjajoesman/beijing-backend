...
# Filtration ("container") pH simulation — server-authoritative, same
# approach as pipe_simulation_mode above, so it works with zero filtration
# controllers connected. When a controller *is* connected, SIM_* commands
# are still forwarded to it (best-effort) so the physical pump/valve keep
# responding for the demo, but the device's own pH telemetry is ignored
# while simulating, since the server's simulated value is what viewers see.
# pump_on/valve_open reported by the device are still trusted regardless of
# simulation mode, since those reflect real actuator hardware, not pH.
CONTAINER_SIM_PH_STEP = 0.1
CONTAINER_SIM_PH_START = 7.0

container_simulation_mode = False
simulated_ph_value = CONTAINER_SIM_PH_START
...
def current_filtration_state() -> dict:
    return {
        ...
        "ph": latest_ph,
        "simulated": container_simulation_mode,
    }


def set_container_simulated_ph(value: float):
    global latest_ph
    latest_ph = {"voltage": None, "value": round(value, 2), "updated_at": datetime.now().isoformat()}


async def handle_sim_command(command: str):
    global container_simulation_mode, simulated_ph_value

    if command == "SIM_ON":
        container_simulation_mode = True
        simulated_ph_value = CONTAINER_SIM_PH_START
        set_container_simulated_ph(simulated_ph_value)
    elif command == "SIM_OFF":
        container_simulation_mode = False
    elif command == "SIM_INC":
        simulated_ph_value = min(14.0, simulated_ph_value + CONTAINER_SIM_PH_STEP)
        if container_simulation_mode:
            set_container_simulated_ph(simulated_ph_value)
    elif command == "SIM_DEC":
        simulated_ph_value = max(0.0, simulated_ph_value - CONTAINER_SIM_PH_STEP)
        if container_simulation_mode:
            set_container_simulated_ph(simulated_ph_value)

    await send_container_command(command)  # best-effort; no-op if no controller connected
    broadcast_filtration_state()
    print(f"[container] [state] Simulation mode is now {'ON' if container_simulation_mode else 'OFF'}")
...
async def handle_container_client(websocket):
    global pump_is_on, valve_is_open
        ...
        async for message in websocket:
            ...
            if data.get("type") == "command":
                ...
                if command in ("SIM_ON", "SIM_OFF", "SIM_INC", "SIM_DEC"):
                    ...
                    await handle_sim_command(command)
                continue

            pH = data.get("pH")
            if pH is not None and not container_simulation_mode:
                latest_ph["voltage"] = pH.get("voltage")
                latest_ph["value"] = pH.get("value")
                latest_ph["updated_at"] = datetime.now().isoformat()

            # pump_on/valve_open reflect real actuator hardware, so these are
            # trusted even while a server-side pH simulation is running.
            if "pump_on" in data:
            ...
            print(
                ...
                f"{'(SIMULATED)' if container_simulation_mode else ''} "
...
async def toggle_simulation():
    await handle_sim_command("SIM_OFF" if container_simulation_mode else "SIM_ON")


async def step_simulated_ph(increase: bool):
    await handle_sim_command("SIM_INC" if increase else "SIM_DEC")
    print(f"[container] [state] Simulated pH {'increased' if increase else 'decreased'} by 0.1")
    if not container_simulation_mode:
